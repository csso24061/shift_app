...
本セクションでは、開発されたWebアプリケーション（MVP版）の具体的な機能構成、画面の操作方法、および今回追加された機能（提出期限のアラート制御仕様）について解説します。

---

### 🖥️ 3.1 画面構成と機能一覧

システムは、ログインしたユーザーのロール（「店舗管理者」または「一般スタッフ」）に応じて、表示するUIや実行可能なアクションを動的に切り替えます。

```mermaid
flowchart TD
    Login[👤 ログイン画面] --> Auth{ロール判定}
    Auth -->|manager| ManagerUI[⚙️ 管理者ダッシュボード]
    Auth -->|staff| StaffUI[📱 スタッフダッシュボード]

    subgraph ManagerUI [管理者専用機能]
        M1[⚙️ システム設定変更] -->|期限・締め日・支給日設定| M_DB[(設定保存)]
        M2[📅 代理登録・カレンダー確認]
        M3[📋 全スタッフのシフト承認・編集・削除]
        M4[📊 全体人件費・配置時間サマリー]
    end

    subgraph StaffUI [一般スタッフ専用機能]
        S1[🚨 提出期限アラートバナー]
        S2[📄 確定勤怠・給与明細確認]
        S3[📅 30分刻みの希望シフト申請]
        S4[📋 申請済みシフト確認・キャンセル]
        S5[💰 今月の概算給与・総労働時間サマリー]
    end
#### 1. 共通機能
*   **ロール自動判定ログイン:**
    *   ログイン時、システムがユーザーIDの権限フラグを判別し、管理者画面またはスタッフ画面へ自動的にリダイレクトします。
*   **イベント・メモの共有表示:**
    *   特定の日付に対して全体メモ（「セール」「棚卸し」など）を登録・表示し、店舗全体でスケジュール感を共有できます。

#### 2. 店舗管理者専用機能（Manager）
*   **システム動作環境の設定:**
    *   「シフト提出期限（例: 25日）」「締め日」「給与支給日」など、運用のコアとなる日付設定をダッシュボードから一括保存・変更できます。
*   **シフト調整・編集・削除:**
    *   提出された希望シフトに対して30分刻みで時間を微調整し、ワンクリックで「確定」ステータスに更新できます。
*   **店舗全体の人件費・稼働サマリー表示:**
    *   選択された月における全スタッフの「総配置時間」と「概算人件費合計」を自動算出。予算管理をリアルタイムに支援します。

#### 3. 一般スタッフ専用機能（Staff）
*   **直感的な30分刻みシフト申請:**
    *   カレンダーの日付を選択し、開始時刻と終了時刻を30分単位で柔軟に指定して希望シフトを申請できます。
*   **シフト確定実績・給与明細確認:**
    *   前月確定した稼働データから、総実働時間、基本給、深夜手当、残業手当、および差引支給額を算出した「Web給与明細」を自動生成・表示します。
*   **今月の稼働概算サマリー:**
    *   現時点で確定している当月の「総稼働時間」と、それに基づいた「概算給料合計」をトップ画面にリアルタイム表示します。
*   **1週間前自動ロック（変更制限）:**
    *   当日から数えて1週間（7日）以内のシフトは、スタッフ側の画面から編集・キャンセルボタンが自動的に非活性化（ロック）され、急な変更を抑止します。

---

### 🚨 3.2 提出催促アラートの動的制御ロジック

一般スタッフの画面上部に表示される「シフト提出期限の接近を知らせる警告バナー」は、店舗管理者が設定した日付に応じて**完全自動で表示・非表示が切り替わる**ロジックを搭載しています。

#### 📅 動作仕様
1.  **期限の3日前からバナーが出現:**
    *   店長が設定した提出期限日（例: `25日`）の**3日前（22日 00:00）**になった瞬間、スタッフ画面の上部に赤色の警告バナーが自動出現します。
2.  **期限当日に自動消去:**
    *   期限日を過ぎる（例: `26日 00:00`）と、提出忘れを防ぐ役割を終えたバナーは自動的に画面から消失します。

#### 🛠️ フロントエンドでの検出制御コード（JavaScript）
```javascript
// 管理者設定から数字だけを抽出（例: 「25日」または「25」から「25」を取得）
const deadlineDayNum = parseInt(systemDeadline.replace(/[^0-9]/g, ''), 10);

// 当月の期限日のDateオブジェクトを生成し、本日の日付と比較
const deadlineDate = new Date(currentYear, currentMonth, deadlineDayNum);
const diffTime = deadlineDate - today;
const diffDays = Math.round(diffTime / (1000 * 60 * 60 * 24));

// 3日前（残り3日）から当日（残り0日）までの期間のみ警告文を表示
