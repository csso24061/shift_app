...
本プロトタイプシステムをローカル環境で起動し、実際に操作（使い方）を検証するための手順を定義します。

##### A. 動作環境の要件
* **検証ブラウザ**: HTML5, CSS3, ES6以降のJavaScriptが動作する主要なWebブラウザ（Google Chrome, Microsoft Edge, Safari等の最新版）。
* **環境依存の排除**: データベースサーバー、バックエンドWebサーバー（Node.js、Apache等）の立ち上げは一切不要です。追加のライブラリインストールなしで動作します。

##### B. 起動手順
1. **ファイルの用意**: 
   GitHub Desktop等を用いて、ローカルPCの任意の場所に本リポジトリをクローン（またはダウンロード）します。
2. **ブラウザで直接起動**: 
   ルートフォルダ直下にある `index.html` ファイルをブラウザへ直接ドラッグ＆ドロップ、もしくはファイルをダブルクリックして開きます。ローカルファイル（`file:///`）としてアプリが即時に立ち上がります。
3. **初期化完了**: 
   ブラウザがページロードを検知すると、JavaScriptが自動的にデモ用データ（初期設定パラメータ、ダミースタッフアカウント情報等）を読み込み、操作可能な初期状態に移行させます。

##### C. 動作方法（使い方）と評価手順
1. **ロール（権限）の動的な切り替え**:  
   画面最上部（ヘッダー領域）にあるトグルボタンを操作し、「店舗管理者パネル」と「一般スタッフパネル」の表示切り替えができることを確認します。
2. **管理者操作（パラメータ設定とシフト確定）**:  
   「店舗管理者」表示にして、シフト提出期限日（例：25日）をフォームに入力して保存します。また、スタッフから届いているシフト一覧テーブルから、承認待ち（`pending`）データを選択し、「確定（`approve`）」ボタンを押下して処理します。
3. **スタッフ操作（申請と給与確認）**:  
   「一般スタッフ」表示に切り替え、30分刻み入力フォームから希望日付・時間帯を選択してシフトを申請します。シフト確定時には、スタッフ用のダッシュボード上で合計稼働時間と自動算出された「当月概算給与（深夜・残業含む）」が動的に更新されることを確認します。
4. **テストデータのクリア（リセット）**:  
   本システムはメモリ内のデータ（Local State）のみで状態を制御しています。最初からやりないしたい場合は、ブラウザの「ページ更新（F5キー / リロード）」を行えば、データは一瞬で初期のクリーンな状態にリセットされます。