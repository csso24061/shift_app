...
* **環境依存の排除**: データベースやWebサーバー（Node.js、Apache等）の立ち上げは不要です。追加のライブラリビルドやインストールなしで動作します。

##### B. 起動手順
1. **ファイルの用意**:  
   GitHub Desktop等を用いて、ローカルPCの任意の場所に本リポジトリをクローン（またはダウンロード）します。
2. **ブラウザでの直接起動**:  
   ルートフォルダ直下にある `index.html` ファイルを動作環境ブラウザへ直接ドラッグ＆ドロップ、もしくはファイルをダブルクリックして開きます。
3. **初期化完了**:  
   ページロードと同時に、JavaScriptが自動的にデモ用データ（初期設定パラメータ、ダミースタッフアカウント情報等）をメモリ上に展開し、システムが実行可能状態に移行します。

##### C. 動作方法（使い方）と評価手順
1. **ロール（権限）の切り替え**:  
   画面最上部にあるトグルスイッチを操作し、「店舗管理者パネル」と「一般スタッフパネル」の表示切り替えができることを確認します。
2. **管理者操作**:  
   「店舗管理者」表示の状態で、シフト提出期限日（例：25日）を入力・保存します。また、承認待ち（`pending`）の希望シフトに対して「確定（`approve`）」ボタンを押下します。
3. **スタッフ操作**:  
   「一般スタッフ」表示に切り替え、30分刻みの入力フォームから希望日付・時間帯を選択してシフトを申請します。シフト確定時には、ダッシュボード上の「当月概算給与」が自動計算され反映されることを確認します。
4. **データの初期化（リセット）**:  
   本システムはメモリ内のデータ（Local State）のみで動作しています。最初からやり直したい場合は、ブラウザの「ページ更新（F5キー / リロード）」を行えば、一瞬で初期状態にリセットされます。